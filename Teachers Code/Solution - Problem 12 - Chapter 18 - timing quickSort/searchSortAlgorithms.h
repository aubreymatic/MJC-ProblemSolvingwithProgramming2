...
template <class elemType>
int partition(elemType list[], int first, int last) // used with quickSort
{
    elemType pivot;

    int index, smallIndex;

    swap(list, first, (first + last) / 2); // swap the first and middle element of sublist

    pivot = list[first]; // pivot is the middle element (remember the swap?)
    smallIndex = first;
...
void recQuickSort20(elemType list[], int first, int last)
{
	...
	if (last-first > 20) // only call quicksort if sublist is greater than 20 elements
	{
		pivotLocation = partition(list, first, last);
		recQuickSort(list, first, pivotLocation - 1);
		recQuickSort(list, pivotLocation + 1, last);
	}
	else // sort sublist if it has 20 elements or less
...
void recQuickSortMedian(elemType list[], int first, int last)
{
	...
	if (first < last)
	{
		pivotLocation = partitionMedian(list, first, last); // pivot will become median of sublist
		recQuickSort(list, first, pivotLocation - 1);
		recQuickSort(list, pivotLocation + 1, last);
	}
}
...
template <class elemType>
int partitionMedian(elemType list[], int first, int last) // used with quickSort with pivot as median
{
	elemType pivot;
...
template <class elemType>
int medianLoc(elemType list[], int first, int middle, int last) // used with quickSort changing pivot to median
{ // algorithm that determines median of sublist
