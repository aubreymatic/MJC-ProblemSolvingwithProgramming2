...
{
	/*
	I need to set first and last to the first node and the last node.
	I need to set subList.first and subList.last to the first and last nodes of the sublist.
	Set current to first.
	Get the integer from the user that wants to seperate the list at.
	Create a for loop that traverses the list to find the node data specified by the user.
	Set current to the variable the user wants to be seperated at.
	set the node link, from the node before the node the user wants to seperate, to null.
	Set the seccond sublist's first node to the number specified by the user. 
	In the testing CPP file, display the two lists.
	*/
}
#endif
